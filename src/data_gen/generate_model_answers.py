...
def load_data_triviaqa(test=False, legacy=False):
    if legacy:
        ...
        """
        Preferred: load from local TriviaQA-unfiltered jsons if present.
        Fallback: download via HuggingFace `datasets` (trivia_qa) if local files are missing.
        Returns:
          - questions: list[str]
          - labels: list[list[str]]  (aliases)
        """
        if test:
        ...
        if os.path.exists(file_path):
            with open(file_path) as f:
                ...
                # keep parity with existing codepaths (cap at 10k for speed)
                if len(data) > 10000:
                ...
                for ex in data:
                    ...
                    if isinstance(ans, dict):
                        als = ans.get('Aliases')
                    if q is None or als is None:
                        continue
                    if isinstance(als, str):
                        als = [als]
                    als = [str(a) for a in als if a is not None and str(a).strip() != ""]
                    if not als:
                        continue
                ...
                if len(questions) == 0:
                    raise RuntimeError(f"Loaded local TriviaQA json ({file_path}) but got 0 usable examples.")

                return questions, aliases

        # --- HF fallback ---
        # Try a few known configurations; different versions of `datasets` expose slightly different configs.
        split = "validation" if test else "train"
        last_err = None
        for cfg in ["unfiltered", "rc", None]:
            try:
                ds = load_dataset("trivia_qa", cfg, split=split)
                # downsample for parity with existing code
                if len(ds) > 10000:
                ...
                for ex in ds:
                    ...
                    # `answer` is usually a dict with key `aliases`
                    als = None
                    if isinstance(ans, dict):
                        als = ans.get("aliases") or ans.get("Aliases")
                    if q is None or als is None:
                        continue
                    # ensure list[str]
                    if isinstance(als, str):
                    ...
                    aliases.append(als)
                if len(questions) == 0:
                    raise RuntimeError("Loaded TriviaQA but got 0 usable examples (missing question/aliases).")
                return questions, aliases
        ...
        raise FileNotFoundError(
            f"TriviaQA local files not found at {file_path}, and HuggingFace download failed: {last_err}\n"
            f"Fix options:\n"
            f"  - Put TriviaQA jsons under ../data/triviaqa-unfiltered/\n"
            f"  - Or ensure internet access + `datasets` can download `trivia_qa`.\n"
        )
